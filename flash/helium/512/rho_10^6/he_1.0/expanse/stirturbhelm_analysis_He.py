...
#plt.plot (timearr, enucarr)
  ...
  #C and He
  plt.plot (timearr, c12arr / rhoarr)
  plt.plot (timearr, he4arr / rhoarr, color='r', linestyle='--')
  #fig, ax = plt.subplots()
  #line1, = ax.plot([3, 2, 1], marker='o', label='Line 1')
  #line2, = ax.plot([1, 2, 3], marker='o', label='Line 2')
  #ax.legend(handler_map={line1: HandlerLine2D(numpoints=4)})
  
  plt.legend(['$^{12}$C', '$^{4}$He'])

  plt.xlabel ('Time (s)')
  plt.ylabel ('Mean $^{12}$C & $^{4}$He Fraction')
  plt.savefig (directory + '_plots/' + 'c12_v_he4.png', format = 'png')
  plt.clf()
