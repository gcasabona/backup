...
real, save    :: L_box, k_min, k_max, pi
...
!!******************************************************
  ...
  ! =========================== user changes ============================
  ! =====================================================================
  ! rtf 4/24/17 Set for StirTurbHelm default setup.
  ! rtf 7/8/17  Modified for variable box size
  ! rtf 6/29/18 Default parameters set to Fisher et al, 2018 values

  ndim = 3;                ! 3-dimensional forcing
  
!  xmin = 0.; xmax = 1.e7; ! spatial x-coordinates of the box (xmax-xmin=L_box)
!  ymin = 0.; ymax = 1.e7; ! spatial y-coordinates of the box (ymax-ymin=L_box)
!  zmin = 0.; zmax = 1.e7  ! spatial z-coordinates of the box (zmax-zmin=L_box)
                           ! Must be adjusted to physical problem

  L_box = 1.e2 * 1.e8    ! set box size in cm

  xmin = -L_box/2.; xmax = L_box/2.; ! spatial x-coordinates of the box (xmax-xmin=L_box)
  ymin = -L_box/2.; ymax = L_box/2.; ! spatial y-coordinates of the box (ymax-ymin=L_box)
  zmin = -L_box/2.; zmax = L_box/2.  ! spatial z-coordinates of the box (zmax-zmin=L_box)
                           ! Must be adjusted to physical problem
  ...
  st_energy    = 6.25e14 !5.e15               ! driving amplitude ~ sqrt(energy/decay)
  ...
  pi = 4. * atan (1.)
  st_stirmax = k_max * 2. * pi / L_box
  st_stirmin = k_min * 2. * pi / L_box
 
!  st_stirmax = 0.00000251327
!  st_stirmin = 6.2832e-07
...
!  st_stirmax   = 18.95               ! >~ 6 pi / L_box ; k=3.0
  ...
  st_solweight = 0.5                 ! 1.0 solenoidal; 0.0 compressive; 0.5 natural mixture
                                     ! See Federrath et al. (2010, A&A 512, A81) for details
                                     ! Must be adjusted to physical problem

  st_seed      = 250618              ! random seed was 140281

  end_time     = 1.6                  ! forcing table end time
                                     ! Must be adjusted to physical problem

  ! use rule-of-thumb to sample the driving pattern 10 times per coherence time
  nsteps       = 160. * end_time / st_decay  ! total number of time dumps to update the
                                     ! forcing pattern until end_time
                                     ! Must be adjusted to physical problem

  outfilename  = 'forcingfile_1600ms_1000km.dat'   ! output filename read by FLASH

  print_stuff_to_shell = .true.     ! print additional information
  ! =====================================================================
