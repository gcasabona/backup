...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
	...
	//Matrix Mpad = AllocateMatri
	//int Mw = M.width;
	//int Mh = M.height;
	//int Nw = N.width;
	//int Nh = N.height;

	// Can Pad M, N and P matrices to be square before sending to device.
	//while (Mw % 32){
//		printf("M width = %d.\n", Mw);	
//		Mw++;
//	}
...
//	}
	...
	//	printf("M width final = %d.\n", Mw);
	// Setup the execution configuration
	// Loop over the d_M and d_N tiles required to compute d_P element
	int NumBlocksW = P.width/tileWidth;
	int NumBlocksH = P.height/tileWidth;

	int tempW = NumBlocksW;
	int tempH = NumBlocksH;


	if ( (P.width % tileWidth)>0) {
		tempW = NumBlocksW + 1;
		printf("Executing!\n");
	}
	if ( (P.height % tileWidth)>0){
	 	tempH = NumBlocksH + 1;
	}

	printf("Num blocks (x,y): (%d, %d)\n", tempW, tempH);
	dim3 dimGrid(tempW, tempH);
	dim3 dimBlock(tileWidth, tileWidth);

	// Launch the device computation threads!
	// BEV: can iteratively launch kernels to complete P matrix, or have threads calculate multiple P elements
