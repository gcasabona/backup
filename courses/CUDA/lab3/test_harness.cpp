...
int main(int argc, char* argv[])
{
    ...
    // BEV: need to pass block and grid dimensions to function
    // BEV: need to allocate device memory for the input and result --> function call with cudaMalloc
    // BEV: need to copy matrics to the devcice to something in .cu file --> function that uses  cudamemcopy

    uint32_t *input_device = AllocateDeviceMemData();
    uint8_t *kernel_bins_device = AllocateDeviceMemResult();
    
    CopyToDevice(input_device, input, kernel_bins_device, kernel_bins);

    // BEV: determine number of blocks
    int blockWidth = 16;
    int blockHeight = 16;

    int numBlocksW = INPUT_WIDTH/blockWidth;
    if (INPUT_WIDTH % blockWidth) numBlocksW++;
    int numBlocksH = INPUT_HEIGHT/blockHeight;
    if (INPUT_HEIGHT % blockHeight) numBlocksH++;
    ...
    TIME_IT("opt_2dhisto",
            1000,
            opt_2dhisto(numBlocksW, numBlocksH, blockWidth, blockHeight, input_device, kernel_bins_device);)

    /* Include your teardown code below (temporary variables, function calls, etc.) */
    // BEV: call function to copy the result from the device back to kernel_bins.
    CopyFromDevice(kernel_bins, kernel_bins_device);

    // BEV: free device memory --> call function to free input and histo result from device memory in .cu file
    FreeCudaMemData(input_device);
    FreeCudaMemResult(kernel_bins_device);
